...
goog.require('Genetics.Tutor.soy');
 ...
 * @private {boolean}
 */
Genetics.Tutor.blocksEnabled_ = true;
 ...
 * @private {boolean}
 */
Genetics.Tutor.ignoreEditorChanges_ = true;

/**
 * Initialize Blockly and/or Ace, and genetics.  Called on page load.
 */
Genetics.Tutor.init = function() {
  // Render the Soy template.
  document.body.innerHTML = Genetics.Tutor.soy.start({}, null,
  ...
  if (editorDiv) {
    ...
    session['on']('change', Genetics.Tutor.editorChanged);
    if (defaultCode) {
  ...
  Genetics.Tutor.blocksEnabled_ = blocklyDiv != null;
  ...
  for (var playerData, i = 0; playerData = players[i]; i++) {
    if (playerData.code) {
      ...
      var code = function() {
        if (Genetics.Tutor.blocksEnabled_) {
  ...
  Genetics.reset();
  if (tabDiv) {
    Genetics.Tutor.changeTab(0);
    Genetics.Tutor.ignoreEditorChanges_ = false;
  }
};

window.addEventListener('load', Genetics.Tutor.init);

/**
 * Called by the tab bar when a tab is selected.
 * @param {number} index Which tab is now active (0-2).
 */
Genetics.Tutor.changeTab = function(index) {
  var BLOCKS = 0;
  var JAVASCRIPT = 1;
 ...
 */
Genetics.Tutor.editorChanged = function() {
  if (Genetics.Tutor.ignoreEditorChanges_) {
    return;
  }
  if (Genetics.Tutor.blocksEnabled_) {
    if (!BlocklyGames.workspace.getTopBlocks(false).length ||
      ...
      Genetics.Tutor.blocksEnabled_ = false;
    } else {
